...
namespace Workstation.UaClient.IntegrationTests
{
    public class IntegrationTests
    {
        ...
        /// <summary>
        /// Tests read structure.
        /// Only run this test with a running opc test server.
        /// </summary>
        /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
        [Fact]
        public async Task ReadVector()
        {
            var channel = new UaTcpSessionChannel(
                localDescription,
                certificateStore,
            ...
            var channel2 = new UaTcpSessionChannel(
                ...
                certificateStore,
                new AnonymousIdentity(),
                "opc.tcp://192.168.1.94:4840",
                loggerFactory: loggerFactory);

            await channel2.OpenAsync();
            logger.LogInformation($"Opened session with endpoint '{channel2.RemoteEndpoint.EndpointUrl}'.");
            logger.LogInformation($"SecurityPolicy: '{channel2.RemoteEndpoint.SecurityPolicyUri}'.");
            logger.LogInformation($"SecurityMode: '{channel2.RemoteEndpoint.SecurityMode}'.");
            logger.LogInformation($"Activated session '{channel2.SessionId}'.");

            var readRequest = new ReadRequest { 
                NodesToRead = new[] { 
                    new ReadValueId { 
                        NodeId = NodeId.Parse("ns=2;s=Demo.Static.Scalar.Vector"), 
                        AttributeId = AttributeIds.Value 
                    } 
                } 
            };
            var readResult = await channel.ReadAsync(readRequest);
            var v1 = (ExtensionObject)readResult.Results[0].Value;
            ExtensionObject v2;
            ...
            var writeRequest = new WriteRequest
            {
                NodesToWrite = new[] {
                    new WriteValue {
                        NodeId = NodeId.Parse("ns=3;s=\"TestData\".\"TestVector\""),
                        AttributeId = AttributeIds.Value,
                        Value = new DataValue(v2)
                    }
                }
            };
            var writeResponse = await channel2.WriteAsync(writeRequest);
