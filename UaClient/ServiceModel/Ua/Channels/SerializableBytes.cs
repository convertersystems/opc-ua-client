...
namespace Workstation.ServiceModel.Ua.Channels
{
    ...
    internal static class SerializableBytes
    {
        ...
        public static PooledStream CreateReadableStream(byte[] bytes, CancellationToken cancellationToken)
        {
        ...
        public static PooledStream CreateReadableStream(Stream stream, CancellationToken cancellationToken)
        {
            return CreateReadableStream(stream, /*length*/ -1, cancellationToken);
        }

        public static PooledStream CreateReadableStream(Stream stream, long length, CancellationToken cancellationToken)
        {
            if (length == -1)
            {
        ...
        public static Task<PooledStream> CreateReadableStreamAsync(Stream stream, CancellationToken cancellationToken)
        {
            return CreateReadableStreamAsync(stream, /*length*/ -1, cancellationToken);
        }

        public static async Task<PooledStream> CreateReadableStreamAsync(Stream stream, long length, CancellationToken cancellationToken)
        {
            if (length == -1)
            {
                length = stream.Length;
        ...
        public static PooledStream CreateWritableStream()
        {
        ...
        public class PooledStream : Stream
        {
                ...
                try
                {
                    switch (origin)
                    {
                        ...
                        case SeekOrigin.Current:
                            target = checked(offset + this.position);
                            break;

                        case SeekOrigin.End:
                            target = checked(offset + this.length);
                            break;
            ...
            public override int ReadByte()
            {
                ...
                var currentIndex = this.CurrentChunkIndex;
                var chunk = this.chunks[currentIndex];

                var currentOffset = this.CurrentChunkOffset;
        ...
        private class ReadWriteStream : PooledStream
        {
            ...
            public override void WriteByte(byte value)
            {
                ...
                this.chunks[currentIndex][currentOffset] = value;
